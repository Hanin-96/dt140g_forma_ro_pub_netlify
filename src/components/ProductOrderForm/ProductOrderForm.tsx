...
import ContactStyle from '../ContactForm/ContactStyle.module.css';
import LoadingSpinnerStyle from '../LoadingSpinner/LoadingSpinnerStyle.module.css';

//Lucide ikon
import { ChevronDown, ChevronUp, Send } from 'lucide-react';
import type { FormErrors, OrderFormData } from "../../types/FormData";
import { observer } from "mobx-react-lite";
import { productStore } from "../../stores/ProductStore";


const ProductOrderForm = observer(() => {
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const templateParams = {
            ...
            amount: amount,
            product_image: productStore.product?.product_thumbnail || "",
            product_name: productStore.product?.title.rendered || "",
            message: message,
    ...
    const formSubmitSend = (templateParams: any) => {
        ...
        formData.append("Antal: ", templateParams.amount);
        formData.append("Meddelande: ", templateParams.message);
        formData.append("Produkt bild: ", templateParams.product_image);
        formData.append("Produkt namn: ", templateParams.product_name);
    ...
    const toggleForm = (toggleOpen: boolean) => {
        setToggleOrderForm(toggleOpen);
        if (!toggleOpen) {
            setTimeout(() => {
                setIsOrderFormOpen(toggleOpen);
            }, 500);
        } else {
            setIsOrderFormOpen(toggleOpen);
        }
            ...
            <div className={"w-full max-w-[100rem] mx-auto mb-[10rem]"}>
                ...
                {isOrderFormOpen && (
                    <div className="bg-forma_ro_grey">
                        <form onSubmit={handleSubmit} className={`${ContactStyle.contactFormContainer} ${toggleOrderForm ? ContactStyle.contactFormSlideDown : ContactStyle.contactFormSlideUp} shadow-lg`}>
                            <div className="p-8">
